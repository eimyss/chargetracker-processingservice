        ...
        stage('Build image') {
            container('docker') {
                ...
                print 'App is:     ${APP}'
                    ...
                    sh 'docker build . -t eimyss/expenses-processing:1.$BUILD_NUMBER'
                ...
                sh 'docker tag eimyss/expenses-processing:1.$BUILD_NUMBER docker-registry.eimantas.server/expenses-processing:1.$BUILD_NUMBER'
                sh 'docker push docker-registry.eimantas.server/expenses-processing:1.$BUILD_NUMBER'
            }
        }
            ...
            stage('Kube update deployment') {
                withCredentials([string(credentialsId: 'kube-ca', variable: 'PW1')]) {
                    withKubeConfig(caCertificate: '${PW1}', clusterName: 'kubernetes', contextName: 'jenkins', credentialsId: '37047961-56be-4bf1-a36e-f4651a546fe4', serverUrl: 'https://192.168.123.243:6443') {
                        ...
                        sh 'kubectl --insecure-skip-tls-verify="true" set image deployment.v1.apps/processing processing=docker-registry.eimantas.server/expenses-processing:1.$BUILD_NUMBER --record=true -n expenses'
